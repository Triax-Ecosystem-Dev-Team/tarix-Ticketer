...
import { 
  ...
  Info, ExternalLink, FileDown, Loader2
} from 'lucide-react';
...
const TripReport = () => {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(`Trip_Report_${trip.id.substring(0, 8)}.pdf`);
    } catch (error) {
  ...
  const handleExportManifestCSV = () => {
    if (!trip || trip.manifest.length === 0) return;
    
    const headers = ['Seat', 'Passenger Name', 'Ticket ID', 'Phone', 'Status'];
    const rows = trip.manifest.map(p => [
      p.seat, 
      p.name, 
      p.ticketId, 
      p.phone, 
    ...
    link.setAttribute('download', `Manifest_${trip.id}.csv`);
  ...
  if (tripLoading || !trip) {
    return (
      <div className="p-6 font-sans bg-[#F8FAFC] min-h-screen flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-slate-500 font-medium">Loading trip report…</p>
      </div>
    );
  }

  const REVENUE_DATA = [
    { name: 'Ticket Sales', value: trip.grossRevenue || 0, color: '#0EA5E9' },
    { name: 'Add-ons', value: 0, color: '#EC4899' },
  ];
          ...
          <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-10 relative z-10">
            <div>
              <div className="flex flex-wrap items-center gap-4 mb-4">
                <h1 className="text-[34px] lg:text-[40px] font-bold tracking-tight">
                  Trip #{trip.id.substring(0, 12)} - {trip.status}
                </h1>
                <span className={clsx(
                  "text-white px-5 py-2 rounded-full text-[13.5px] font-extrabold flex items-center gap-2 border-2 border-white/20 shadow-lg",
                  trip.status === 'Completed' ? 'bg-[#22C55E]' : 'bg-[#F59E0B]'
                )}>
                  {trip.status === 'Completed' && <CheckCircle className="w-4.5 h-4.5" />}
                  {trip.status}
                </span>
              </div>
              <div className="flex flex-wrap items-center gap-6 text-white/90 font-bold tracking-tight">
                <span className="text-[19px] lg:text-[22px]">{trip.from} → {trip.to}</span>
                <div className="hidden sm:block w-2 h-2 rounded-full bg-white/25" />
                <span className="text-[17px] lg:text-[18px]">
            ...
            <div className="flex items-center gap-4" data-html2canvas-ignore>
              <button 
                ...
                className="flex items-center gap-3 px-8 py-4 bg-white text-[#0EA5E9] rounded-2xl font-bold text-[15px] hover:bg-white/95 transition-all shadow-xl shadow-black/5 active:scale-95 disabled:opacity-70"
              >
        ...
        <div className="bg-white rounded-[40px] p-10 lg:p-12 border border-slate-100 shadow-sm mb-10 overflow-hidden">
          ...
          <div className="relative pt-4 pb-8">
            ...
            <div className="grid grid-cols-4 relative z-10 font-sans">
              {[
                { label: 'Scheduled', time: trip.time, icon: Clock, color: 'text-[#0EA5E9]', bg: 'bg-[#E0F2FE]' },
                { label: 'Departed', time: trip.time, icon: Bus, color: 'text-[#0EA5E9]', bg: 'bg-[#E0F2FE]' },
                { label: 'In Transit', time: '—', icon: Activity, color: 'text-[#0EA5E9]', bg: 'bg-[#E0F2FE]' },
                { label: 'Arrived', time: trip.status === 'Completed' ? 'Completed' : 'TBD', icon: CheckCircle, color: trip.status === 'Completed' ? 'text-[#22C55E]' : 'text-slate-300', bg: trip.status === 'Completed' ? 'bg-[#DCFCE7]' : 'bg-slate-50', active: trip.status === 'Completed' },
              ].map((step, i) => (
                <div key={i} className="flex flex-col items-center">
                  <div className={clsx(
                    "w-16 h-16 rounded-full border-[5px] border-white shadow-md flex items-center justify-center mb-6 cursor-default",
                    step.bg
                  )}>
            ...
            <div className="space-y-12">
              <div>
                ...
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <h4 className="text-[20px] font-bold text-[#1E293B]">{trip.from} → {trip.to}</h4>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-5 ml-2">
                  {[
                    { label: 'Scheduled Departure:', value: trip.time },
                    { label: 'Actual Departure:', value: trip.time, color: 'text-[#D97706]' },
                    { label: 'Scheduled Arrival:', value: 'TBD' },
                    { label: 'Actual Arrival:', value: trip.status === 'Completed' ? 'Completed' : 'TBD', color: 'text-[#22C55E]' },
                  ].map((row, i) => (
            ...
            <div className="space-y-12 lg:border-l lg:border-slate-50 lg:pl-16">
              <div>
                ...
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <h4 className="text-[22px] font-bold text-[#1E293B]">{trip.driver}</h4>
                    <a href={`tel:${trip.driverPhone}`} className="flex items-center gap-2 text-[#0EA5E9] font-bold hover:underline">
              ...
              <div className="pt-4">
                ...
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <h4 className="text-[19px] font-bold text-[#1E293B]">{trip.bus}</h4>
                </div>
                <p className="text-[15px] text-slate-400 font-bold ml-14">Capacity: {trip.busCapacity} seats</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[40px] p-10 lg:p-12 border border-slate-100 shadow-sm mb-10 overflow-hidden font-sans">
          <div className="flex items-center justify-between mb-10">
            <div>
              ...
              <p className="text-[14px] text-slate-400 font-bold">{trip.passengers} passengers booked</p>
            </div>
            <button 
              onClick={handleExportManifestCSV}
              data-html2canvas-ignore
              className="flex items-center gap-2 px-6 py-3 bg-white border border-slate-200 rounded-2xl text-[#0EA5E9] font-bold text-[14px] hover:bg-slate-50 transition-all shadow-sm"
            >
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-50">
                {trip.manifest.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-8 py-8 text-center text-slate-500 font-medium">No passengers found.</td>
                  </tr>
                )}
                {trip.manifest.map((p, i) => (
                  <tr key={i} className="hover:bg-slate-50/50 transition-colors border-b border-slate-50">
                    ...
                    <td className="px-8 py-5 text-slate-400 font-bold text-[14px]">{p.phone}</td>
                    <td className="px-8 py-5">
                      <span className={clsx(
                        "px-3 py-1.5 rounded-xl text-[12px] font-bold shadow-sm inline-block",
                        p.status === 'Checked In' ? "bg-green-50 text-[#16A34A]" : "bg-slate-100 text-slate-500"
                      )}>
                        {p.status}
                      </span>
                    </td>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 mb-10">
          ...
          <div className="bg-white rounded-[40px] p-10 lg:p-12 border border-slate-100 shadow-sm relative pr-16">
            ...
            <div className="space-y-6 pt-2">
              {[
                { label: 'Total Tickets Sold:', value: trip.passengers.toString(), bold: true },
                { label: 'Ticket Price:', value: `₦${(trip.ticketPrice || 0).toLocaleString()}` },
                { label: 'Gross Revenue:', value: `₦${trip.grossRevenue.toLocaleString()}`, color: 'text-[#22C55E]', bold: true },
                { label: 'Deductions:', value: `₦${trip.deductions.toLocaleString()}`, color: 'text-slate-400' },
              ].map((row, i) => (
              ...
              <div className="flex justify-between items-center pt-8 mt-4 border-t-2 border-slate-50">
                ...
                <span className="text-[28px] font-black text-[#22C55E] tracking-tight">₦{trip.netRevenue.toLocaleString()}</span>
              </div>
            </div>
          </div>
                ...
                {REVENUE_DATA.map((item, i) => (
                  <div key={i} className="flex items-center gap-4">
                    ...
                    <div className="min-w-0">
                      <p className="text-[15px] font-bold text-slate-500 truncate leading-none mb-1.5">{item.name}</p>
        ...
        <div className="bg-[#FEF9C3]/50 rounded-[40px] p-10 lg:p-12 border-2 border-[#FEF3C7] shadow-sm mb-10 relative overflow-hidden">
          ...
          <div className="max-w-3xl relative z-10">
            ...
            <div className="space-y-6 mb-10">
              {[
                { label: 'Base Earnings:', value: `₦${(trip.driverEarnings - 2280).toLocaleString()}` },
            ...
            <div className="pt-8 border-t-2 border-[#FEF3C7] flex justify-between items-center">
              ...
              <span className="text-[32px] font-black text-[#854D0E]">₦{trip.driverEarnings.toLocaleString()}</span>
            </div>
          </div>
        </div>
      </div>
